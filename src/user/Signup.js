    ...
    const onSubmit = event => {
        ...
        .then(data=>{
            ...
            if(data.error || data.err){
                setValues({...values, error: data.error?data.error:"User already exists", success: false})
            }else{
                setValues({
                    ...values,
                    name: "",
        ...
        return (<div className="row">
            <div className="col-md-6 offset-sm-3 text-left">
                <form action="">
                    ...
                    <button onClick={onSubmit} className="btn btn-success col-12 mt-3">Submit</button>
                </form>
            </div>
        </div>)
    }
